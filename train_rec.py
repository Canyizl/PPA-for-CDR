    ...
    for epoch in range(0, opt["num_epoch"] + 1):
        ...
        for iteration in range(iteration_num):
            ...
            for idx in range(opt["num_domains"]):  # get one batch from each dataloader
                ...
                cur_loss,prop_loss = mymodel.reconstruct_graph(idx, batch_data)
        ...
        if opt["aggregator"] == "Transformer":
            print("Average loss:", sum(total_loss)/len(total_loss), "prop_loss", sum(total_ploss)/len(total_ploss), "time: ", (time.time() - start_time) / 60, "(min) current lr: ",current_lr)
